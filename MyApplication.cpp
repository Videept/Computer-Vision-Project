...
#include <filesystem> // Microsoft-specific implementation header file me
...
public:
	Histogra(Mat image, int number_of_bins)
	{
		mImage = image;
...
class ColourHistogra : public Histogra
{
private:
	MatND mHistogram;
public:
	ColourHistogra(Mat all_images[], int number_of_images, int number_of_bins) :
		Histogra(all_images[0], number_of_bins)
	{
	...
	ColourHistogra(Mat image, int number_of_bins) :
		Histogra(image, number_of_bins)
	{
		ComputeHistogram();
	}
...
class OneDHistogra : public Histogra
{
private:
	MatND mHistogram[3];
public:
	OneDHistogra(Mat image, int number_of_bins) :
		Histogra(image, number_of_bins)
	{
		ComputeHistogram();
	}
...
{
	Mat smaller_image;
	resize(image, smaller_image, Size(image.cols / 4, image.rows / 4));

	
	cv::Mat Hsvimage;
	cv::cvtColor(smaller_image, Hsvimage, COLOR_BGR2HSV);
	...
	Mat thresholded;
	threshold(S, thresholded, 35, 255, cv::THRESH_BINARY_INV || cv::THRESH_OTSU);

	findContours(thresholded, contours, hierarchy, RETR_EXTERNAL, cv::CHAIN_APPROX_SIMPLE);

	cv::Mat outputH = smaller_image.clone();

	for (int i = 0; i < contours.size(); i++)
	{
		approxPolyDP(contours[i], border, arcLength(contours[i], true) * 0.02, true);
		double area = contourArea(contours[i]);

		if ( border.size() == 4 && area > 450)
		{
			int j = 0;
			int xminlocation = border[0].x;
			int yminlocation = border[0].y;
			int xmaxlocation = border[0].x;
			int ymaxlocation = border[0].y;
			while(j<4) 
			{
				if (border[j].x < xminlocation) 
					xminlocation = border[j].x;
				if (border[j].y < yminlocation) 
					yminlocation = border[j].y;
				if (border[j].x > xmaxlocation) 
					xmaxlocation = border[j].x;
				if (border[j].y > ymaxlocation) 
					ymaxlocation = border[j].y;
				//line(outputH, border[j], border[(j + 1) % 4], Scalar(0, 0, 255), 2, LINE_AA, 0);
				j++;
			}
			cv::Rect myROI(xminlocation, yminlocation, xmaxlocation - xminlocation, ymaxlocation - yminlocation);
			cv::Mat croppedref(outputH, myROI);
			cv::Mat cropped;
			croppedref.copyTo(cropped);
			ObjectAndLocation* p = addObject("string", 0, 0, xmaxlocation - xminlocation, 0, xmaxlocation - xminlocation, ymaxlocation - yminlocation, 0, ymaxlocation - yminlocation, cropped);
			training_images.FindBestMatch(p);
		}
	}
	//imshow(filename, outputH);
}

#define BAD_MATCHING_VALUE 1000000000.0;
double ObjectAndLocation::compareObjects(ObjectAndLocation* otherObject)
{
	Mat Object_image = otherObject->image;
	Mat smaller_image;
	resize(image, smaller_image, Size(Object_image.cols, Object_image.rows));

	Mat space;
	space.create(Object_image.cols - smaller_image.cols + 1, Object_image.rows - smaller_image.rows + 1, CV_32FC1);
	matchTemplate(Object_image, smaller_image, space, cv::TM_CCORR_NORMED);
